...
export function CreatePackageForm() {
  ...
  // Helper to find which tab has the error and navigate to it
  const getTabForField = (fieldName: string): string => {
    if (fieldName.startsWith("sender")) return "sender"
    if (fieldName.startsWith("recipient")) return "recipient"
    if (fieldName.startsWith("checkpoints") || fieldName === "tracking") return "tracking"
    if (fieldName === "images" || fieldName.startsWith("images")) return "images"
    return "details"
  }
  ...
  const handleFormError = () => {
    ...
    if (errorKeys.length > 0) {
      ...
      toast.error("Please fill in required fields", {
        description: `Missing required field in ${targetTab} tab`
      })
    }
  }
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end space-x-2 pt-4 border-t">
            ...
            <Button 
              ...
              onClick={() => {
                // Check for errors after a small delay to allow validation
                setTimeout(() => {
